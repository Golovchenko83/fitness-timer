...
long delays = 20;            // задержка нажатия кнопки для антидребезга
long d_t;                    // таймер не обновления информации на дисплее
volatile long antidebounce;  // время нажатия кнопки для антидребезгаvolatile
long InterruptTaimer = 5000; // таймер пробуждения
long FitnessTaimer;          // количество времени Fitness таймера
long UpdateFitnessTaimer;    // таймер не обновления информации Fitness таймера на дисплее
long svoimillis;             // svoimillis=millis()
volatile long nazatie;       // время нажатия кнопки
volatile long otrezok;  // отрезок времени от нажатия и отжатия кнопки
volatile long ClickTaimer;   // таймер для двойного и более нажатия
long ostatok;                // время в миллисекундах до истечения Fitness таймера
...
void Interrupt() // функция прерывания
{
  ...
  if (flagSleep == 0)
  {
    ...
    if (digitalRead(2) && flagNazatia == 1 && (svoimillis - antidebounce) >= delays) // если кнопку отжали после нажатия
    {
      ...
      if (otrezok > 1000) // если кнопку отжали дольше, чем за 400 миллисекунд (удержание)
      {
